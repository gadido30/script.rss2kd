...
class Settings:  # Read Configuration's Addon
    ...
    def log(self, message=""):  # to write in the Kodi's log
        xbmc.log('[%s] %s' % (self.cleanName, message))
...
############  INTEGRATION   ###########################
def integration(filenames=[], magnets=[], titles=[], typeList='', folder='', silence=False, message=''):
    messageType = {'MOVIE': 'MOVIES', 'SHOW': 'EPISODES', 'ANIME': 'EPISODES ANIME'}
    if len(titles) == 0: titles = filenames  # Verify if we have the titles or not
    ...
    answer = True
    if not silence:
        answer = settings.dialog.yesno('%s: %s items\nDo you want to integrate this list?' %
                                       (settings.name, total), '%s' % filenames)
    if answer:  # it will integrate the filename list to the local library
        if not silence:
            settings.pDialog.create(settings.name, 'Checking for %s\n%s' % (messageType[typeList], message))
        else:
            settings.notification('Checking for %s\n%s' % (messageType[typeList], message))
        ...
        for cm, title in enumerate(titles):
            ...
            if check:  # the file has passed the filtering
                ...
                if not os.path.isfile(filename) or settings.value["overwrite"] == 'true':
                    ...
                    if cont % 100 == 0: settings.notification('%s %s found - Still working...\n%s')
                    settings.log('%s added' % (filename))
                if not silence and settings.pDialog.iscanceled(): break
        ...
        if cont > 0:  # There are files added
            ...
            settings.log('%s %s added./n%s' % (cont, messageType[typeList], message))
            if not silence:
                settings.dialog.ok(settings.name, '%s %s added.\n%s' % (cont, messageType[typeList], message))
            else:
                settings.notification('%s %s added.\n%s' % (cont, messageType[typeList], message))
        else:
            settings.log('No new %s\n%s' % (messageType[typeList], message))
            if not silence:
                settings.dialog.ok(settings.name, 'No new %s\n%s' % (messageType[typeList], message))
            else:
                settings.notification('No new %s\n%s' % (messageType[typeList], message))
                # del filters


############  SUBSCRIPTION   ###########################
def subscription(listing=[], ID=[], typeList='', folder='', silence=False, message=''):
    messageType = {'MOVIE': 'MOVIES', 'SHOW': 'EPISODES'}
    total = len(listing)
    answer = True
    if not silence:
        answer = settings.dialog.yesno('%s: %s items\nDo you want to integrate this list?' %
                                       (settings.name, total), '%s' % listing)
    ...
    if answer:  # it will integrate the filename list to the local library
        ...
        for cm, item_list in enumerate(listing):
            ...
            if storage.database.has_key(item):
                ...
                if typeList == 'MOVIE' and data['type'] == 'MOVIE' and data['episode'] == 0 and data['ID'] is not None:
                    ...
                    data['episode'] = 1
                    if not silence: settings.pDialog.update(int(float(cm) / total * 100),
                                                            'Creating %s%s.strm...' % (directory, item))
                    if cont % 100 == 0: settings.notification('%s %s found - Still working...\n%s'
                    ...
                    data['path'] = directory  # To be able to erase the folder
                    if settings.value["detailedLog"] == 'true':
                        settings.log('Code %s=%s\n%s' % (typeList, data['ID'], message))
                        settings.log(
                    ...
                    if not silence: settings.pDialog.update(int(float(cm) / total * 100),
                                                            'Creating %s/%s.strm...' % (directory, item))
                    settings.log('%s/%s.strm added' % (directory, item))
                # update database
                if data['ID'] is not None:
        ...
        if cont > 0:
            if not xbmc.getCondVisibility('Library.IsScanningVideo'):
                xbmc.executebuiltin('XBMC.UpdateLibrary(video)')  # update the library with the new information
            settings.log('%s %s added. %s' % (cont, messageType[typeList], message))
            if not silence:
                settings.dialog.ok(settings.name, '%s %s added.\n%s' % (cont, messageType[typeList], message))
            else:
                settings.notification('%s %s added.\n%s' % (cont, messageType[typeList], message))
            ...
            settings.log('No new %s. %s' % (messageType[typeList], message))
            if not silence:
                settings.dialog.ok(settings.name, 'No new %s\n%s' % (messageType[typeList], message))
            else:
                settings.notification('No new %s\n%s' % (messageType[typeList], message))
